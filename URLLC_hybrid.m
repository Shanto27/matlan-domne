...
if (currentTime==iaarray(k)) % if slot matched
    ...
    Zup=(Zoneselection*20);
    Zlow=((Zoneselection-1)*20)+1;
    
    Channelselection=randi([1,25]);
    if (Channelselection>20)
        fx=fx+100000000;
    else
        fx=[3.8*10^9];
    end
    f=randi([Zlow,Zup]);    % random carrier chnl selection
